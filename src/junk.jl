...
begin 
    ...
    fig
    save("/Users/gardnera/Downloads/1.png", fig)
end
...
begin 
    ...
    axislegend()
    fig
    save("/Users/gardnera/Downloads/2.png", fig)
end


# calculate count/nrow and mean, medain and std of `v1` binned according to `x`
...
begin
    ...
    axislegend()
    fig
    save("/Users/gardnera/Downloads/3.png", fig)
end
...
begin 
    ...
    heatmap!(fig[1, 2], unique(bincenter.(df4[:,1])),unique(bincenter.(df4[:,2])), 
        ...
        colormap = :thermal)
    fig
    save("/Users/gardnera/Downloads/4.png", fig)
end

# calculate 'median' of `v2` binned according to `y` and `x` using non-uniform binstats
...
begin 
    ...
    xlims!(0, 20); ylims!(0, 20)
    Axis(fig[1, 2], title = "binned data")
    heatmap!(fig[1, 2], unique(bincenter.(df5[:,1])),unique(bincenter.(df5[:,2])), reshape(df5.v2_median,length(unique(df5[:,2])),length(unique(df5[:,1]))), colormap = :thermal)
    fig
    save("/Users/gardnera/Downloads/5.png", fig)
end